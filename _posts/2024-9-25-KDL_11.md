...
![image](https://github.com/user-attachments/assets/ab7a5c18-8452-4cf5-86e3-c030927628b9)
...
![image](https://github.com/user-attachments/assets/1ebe11ca-234d-48d3-a578-7f212fea7375)
...
- **원-핫 인코딩**:
    - 대부분이 0이라 **희소**하고, **고차원**인 이진 벡터를 만든다.
        ...
        ![image](https://github.com/user-attachments/assets/8573e1a7-81bd-45fe-9350-4bd96ce8f905)
...
- **단어 임베딩**:
    ...
    - 임베딩 공간에서 의미 있는 기하학적 변환의 일반적인 예
        ...
        ![image](https://github.com/user-attachments/assets/a61b3b29-2d0d-49fc-852b-0eb104c0eaf8)
...
사전 훈련된 단어 임베딩(Pre-trained Word Embeddings)을 사용하는 방법은 여러 가지가 있으며, 주요한 방법으로는 Word2Vec, GloVe, FastText, 그리고 최신 Transformer 기반의 contextual embeddings(BERT, KoBERT 등)이 있습니다.

✅ 1. 사전 훈련된 단어 임베딩 사용 방법

1️⃣ Word2Vec (Google News, 네이버 Word2Vec 등)

Word2Vec은 단어를 벡터 공간에 매핑하는 기술로, gensim 라이브러리를 사용하면 쉽게 적용할 수 있습니다.

```python
import gensim.downloader as api

# Google 사전 훈련된 Word2Vec 모델 로드
model = api.load("word2vec-google-news-300")  # 300차원 벡터
...
⚠ 주의: Word2Vec은 영어 전용이며, 한국어 모델이 필요하면 네이버 Word2Vec 같은 대체 모델을 사용해야 합니다.

2️⃣ GloVe (Stanford)

GloVe는 Word2Vec과 유사하지만, 공분산 행렬 기반으로 임베딩을 학습한 모델입니다.

🔹 GloVe 사전 훈련된 임베딩 사용 (300차원)
...
# GloVe 다운로드 후 적용
glove_path = "glove.6B.300d.txt"  # 로컬에 다운받아야 함
...
⚠ 주의: GloVe도 영어 전용 모델이므로 한국어 데이터에는 적용이 어렵습니다.

3️⃣ FastText (Facebook)

FastText는 단어의 부분(Subword)을 고려하여 OOV(Out of Vocabulary) 문제를 해결하는 모델입니다.

```python
import fasttext.util

# 사전 훈련된 FastText 모델 다운로드
fasttext.util.download_model('en', if_exists='ignore')  # 영어 모델
ft = fasttext.load_model('cc.en.300.bin')

# 단어 벡터 가져오기
vector = ft.get_word_vector("king")
```

단어 수준 임베딩이 필요하면 Word2Vec, GloVe, FastText를 사용.
	•	문맥 정보를 포함하는 문장 임베딩이 필요하면 **BERT 계열(KoBERT, KoELECTRA)**을 활용.
	•	사전 훈련된 임베딩을 적용하려면 Keras Embedding 레이어 또는 PyTorch Tensor로 변환하여 사용 가능.
